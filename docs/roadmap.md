...
Action metadataには外部から受け取るenvの宣言箇所がないため、envは`action_assert.yml`で定義しruntime assertionの対象とする。Action inputsは`action-inputs: ${{ toJSON(inputs) }}`でgh-assertへ明示的に渡す。
